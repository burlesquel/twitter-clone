...
export default function Tweet({ tweet_ }) {

  const router = useRouter()

  const tweet = tweet_?.tweet || tweet_ // IF IT IS A RETWEET, THE FIRST ONE WILL RETURN FALSY
  ...
  return (
    <div className={styles.container}>
      {tweet_.retweet && <span className={styles.retweeted_by}><i className='bi bi-recycle'></i> <span onClick={()=>{router.push(`/profile/${tweet_.interactor_user?.username}`)}}>Retweeted by {tweet_.interactor_user?.username}</span></span>}
      <div className={styles.main}>
        ...
        <div className={styles.content}>
          ...
          <div className={styles.buttons}>
            ...
            <div className={styles.iconNumberContainer}>
              <div onClick={() => { newInteraction(1) }} className={styles.iconContainer}><i className='bi bi-recycle'></i></div>
